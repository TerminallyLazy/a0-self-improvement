...
- Reconciles optimizer output into append-only v3 review candidates and audit receipts immediately after generation and during automatic project setup. These bridge records are structurally ineligible for activation.
...
An Autopilot value saved by an older release remains non-authoritative until the operator selects Autopilot again. That explicit selection records the current authority-consent revision.
...
In version 2.1.0, selecting **Autopilot** is the operator's one-time authorization for the bounded automatic loop. Project setup creates an exact production policy, canary plan, monitor plan, and signed calibration for each project chat. RLM and GEPA remain proposal engines: they can produce only allowlisted structured guidance and cannot mutate active state. A worker proposal becomes eligible only after its current lease and fencing token commit a terminal candidate result. The framework-owned transition runner then digest-verifies the candidate and replay evidence, assigns each complete user message to one deterministic candidate or incumbent arm, and evaluates their failure-rate delta at the fixed calibrated horizon. The horizon limits the combined probability that either arm misses its minimum comparable sample to one percent. Every message has at most one durable outcome receipt, expired canaries terminate, unknown outcomes do not advance the experiment, and ordinary failures remain separate from explicitly structured hard failures. The runner promotes with an active-revision compare-and-swap, monitors the promoted artifact against the measured incumbent arm, and automatically rolls back regressions without overwriting manual pointer changes. Every transition revalidates the candidate publication against the current Activation Scope; every promotion or rollback also requires a short-lived signed grant and revalidation of the exact policy, calibration, plans, context, candidate, action, and revision under an authority-store write fence. Content-free canary, promotion, monitoring, rollback, and recovery states appear in Recent activity. Review and Observe never provision this authority.

The first candidate has no incumbent guidance artifact for a paired replay. It may advance from the explicit `missing_baseline` replay state only into the bounded live canary; all other replay refusals remain blocked. Later candidates require promotion-ready paired replay before canary admission. The current certified system-prompt hook exposes a trusted live bucket only for `reasoning`, so other bucket proposals remain visible for review instead of entering an experiment that cannot receive correctly attributed traffic. Selected-chat counts match the selected chat's detail tabs, while project totals are labeled separately.

Every editable Settings field is normalized into its canonical plugin setting before save and restored from that same setting on reopen. Review and Autopilot visibly lock the capabilities required by the selected mode instead of presenting controls that the mode would overwrite.
